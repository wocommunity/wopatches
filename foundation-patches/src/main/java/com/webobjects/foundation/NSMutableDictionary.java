...
/**
 * <div class="en"> NSMutableDictionary reimplementation to support JDK 1.5
 * templates and the proper collection methods. Use with</div>
 *
...
public class NSMutableDictionary<K, V> extends NSDictionary<K, V> {
	...
	public NSMutableDictionary(final int capacity) {
		this();
		if (capacity < 0) {
	...
	public NSMutableDictionary(final V object, final K key) {
		super(object, key);
	}

	public NSMutableDictionary(final V[] objects, final K[] keys) {
		super(objects, keys);
	}

	public NSMutableDictionary(final NSArray<? extends V> objects, final NSArray<? extends K> keys) {
		super(objects, keys);
	}

	public NSMutableDictionary(final NSDictionary<? extends K, ? extends V> otherDictionary) {
		_copyMutableDictionary(otherDictionary);
	}

	public NSMutableDictionary(final Dictionary<? extends K, ? extends V> dictionary, final boolean ignoreNull) {
		super(dictionary, ignoreNull);
	}

	public NSMutableDictionary(final Map<? extends K, ? extends V> map) {
		super(map);
	}

	public NSMutableDictionary(final Map<? extends K, ? extends V> map, final boolean ignoreNull) {
		super(map, ignoreNull);
	}

	public void setObjectForKey(final V object, final K key) {
		if (object == null) {
	...
	public V removeObjectForKey(final Object key) {
		Object result = null;
		if (key == null) {
	...
	public void setDictionary(final NSDictionary<? extends K, ? extends V> otherDictionary) {
	...
	public void addEntriesFromDictionary(final NSDictionary<? extends K, ? extends V> otherDictionary) {
		if (otherDictionary != null) {
			final Object[] keys = otherDictionary.keysNoCopy();
			for (final Object key : keys) {
				setObjectForKey(otherDictionary.objectForKey(key), (K) key);
			}

		}
	}

	public void removeObjectsForKeys(final NSArray<? extends K> keys) {
		if (keys != null) {
			final Object[] keysArray = keys.objectsNoCopy();
			for (final Object element : keysArray) {
				removeObjectForKey(element);
			}
	...
	public static final Class _CLASS = _NSUtilitiesExtra
			._classWithFullySpecifiedNamePrime("com.webobjects.foundation.NSMutableDictionary");

	/**
	 * Associate the <tt>value</tt> with <tt>key</tt> in this map. If the map
	 * previously contained a mapping for <tt>key</tt>, the old value will be
	 * replaced by <tt>value</tt>.
	 *
	 * @param key   key to associate value with
	 * @param value vaue to be associated with key
	 *
