...
/**
 * <div class="en"> Bugfix reimplementation of NSMutableArray. To be able to use
 * this class, the framework this class resides in must be before
 * JavaFoundation.framework in your classpath.
 * <p>
 * It fixes a lot of issues:
 * <ul>
 * <li>implements the correct Collection methods, so you can
 * <code>anArray.add(anObject)</code>
 * <li>has several performance improvements that make EOF faster by several
 * orders of magnitude when you have large record sets
 * <li>fixes a bug when the actual objects were handed out
 * replaceObjectAtIndex()
 * <li>fixes a bug when the iterator method wasn't firing a fault in
 * <code>_EOCheapCopyMutableArray</code>
 * </ul>
 *
 * Once these issues are resolved in a WO distribution, this class will go away
 * and the Apple supplied will will be used again without changes in code on
 * your side. </div>
 *
 * <div class="ja"> NSMutableArray のバッグフィックス再実装。 このクラスを使用する為には現フレームワークのクラスパスが
 * JavaFoundation.framework の前にある必要があります。
 * <p>
 * 次の問題を対応しています:
 * <ul>
...
public class NSMutableArray<E> extends NSArray<E> implements RandomAccess {
	...
	public static final Class _CLASS = _NSUtilitiesExtra
			._classWithFullySpecifiedNamePrime("com.webobjects.foundation.NSMutableArray");
	...
	public NSMutableArray(final Collection<? extends E> collection) {
		super(collection);
	}

	public NSMutableArray(final int capacity) {
		if (capacity < 0) {
	...
	public NSMutableArray(final E object) {
		super(object);
	}

	public NSMutableArray(final E[] objects) {
		super(objects);
	}

	public NSMutableArray(final E object, final E... objects) {
		super(object, objects);
	}

	public NSMutableArray(final E[] objects, final NSRange range) {
		super(objects, range);
	}

	public NSMutableArray(final NSArray<? extends E> otherArray) {
		super(otherArray);
	}

	public NSMutableArray(final Vector<? extends E> vector, final NSRange range, final boolean ignoreNull) {
		super(vector, range, ignoreNull);
	}

	public NSMutableArray(final List<? extends E> list, final NSRange range, final boolean ignoreNull) {
		...
		if (capacity > _capacity) {
			if (capacity == 0) {
				...
				if (capacity < 4) {
					...
					final int testCapacity = 2 * _capacity;
	...
	public void setArray(final NSArray<? extends E> otherArray) {
		if (otherArray != this) {
			if (otherArray == null) {
				_setCount(0);
			} else {
				final Object[] objects = otherArray.objectsNoCopy();
	 ...
	 */
	@Deprecated
	public void replaceObjectAtIndex(final int index, final E object) {
		replaceObjectAtIndex(object, index);
	}

	public void insertObjectAtIndex(final E object, final int index) {
		if (object == null) {
			throw new IllegalArgumentException("Attempt to insert null into an  " + getClass().getName() + ".");
		}
		final int count = count();
	...
	public E removeObjectAtIndex(final int index) {
		int count = count();
		if (index >= 0 && index < count) {
			count--;
			final Object[] objs = _objects();
			final Object result = objs[index];
			if (index < count) {
	...
	public void sortUsingComparator(final NSComparator comparator) throws NSComparator.ComparisonException {
		if (comparator == null) {
	...
	public void addObjectsFromArray(final NSArray<? extends E> otherArray) {
		if (otherArray != null) {
			addObjects((E[]) otherArray.objectsNoCopy());
		}
	}

	public void replaceObjectsInRange(final NSRange range, final NSArray<? extends E> otherArray,
			final NSRange otherRange) {
	...
	private boolean _removeObject(final Object object, final int index, final int length, final boolean identical) {
	...
	public boolean removeObject(final Object object, final NSRange range) {
		boolean wasRemoved = false;
		if (range != null) {
			final int count = count();
	...
	public boolean removeIdenticalObject(final Object object) {
		return _removeObject(object, 0, count(), true);
	}

	public boolean removeIdenticalObject(final Object object, final NSRange range) {
		if (range != null) {
			final int count = count();
			final int rangeLocation = range.location();
			final int rangeLength = range.length();
	...
	public void removeObjectsInArray(final NSArray<?> otherArray) {
		if (otherArray != null) {
			removeObjects(otherArray.objectsNoCopy());
		}
	}

	public void removeObjectsInRange(final NSRange range) {
		if (range != null) {
			final int count = count();
			final int rangeLocation = range.location();
			int rangeLength = range.length();
	...
	public void _moveObjectAtIndexToIndex(final int sourceIndex, final int destIndex) {
		...
		if (sourceIndex < 0 || sourceIndex >= count || destIndex < 0 || destIndex >= count) {
			throw new IllegalArgumentException(
					"Either source(" + sourceIndex + ") or destination(" + destIndex + ") is illegal.");
		}
		final Object objs[] = _objects();
		final Object temp = objs[sourceIndex];
		int boundary;
		int index;
		int direction;
		if (sourceIndex < destIndex) {
	 ...
	 */
	public E replaceObjectAtIndex(final E object, final int index) {
		if (object == null) {
			throw new IllegalArgumentException("Attempt to insert null into an  " + getClass().getName() + ".");
		}
		final int count = count();
	 ...
	 */
	public void removeObjects(final Object... otherObjects) {
		if (otherObjects != null) {
			final int count = count();
			if (count > 0) {
				final int otherCount = otherObjects.length;
				if (count * otherCount > 100) {
					if (count > 0) {
						final NSMutableSet<Object> table = new NSMutableSet<>(otherCount);
						for (int i = 0; i < otherCount; i++) {
							final Object o = otherObjects[i];
							if (o != null) {
								table.addObject(o);
							}
						}
					...
					for (final Object otherObject : otherObjects) {
	...
	public E set(final int index, final E element) {
		final E old = objectAtIndex(index);
		if (element != old) {
...
class SubList<E> extends NSMutableArray<E> {
	...
	SubList(final NSMutableArray<E> list, final int fromIndex, final int toIndex) {
		if (fromIndex < 0) {
	...
	public String toString() {
		...
		sb.append('(');
		for (int i = 0; i < count; i++) {
			final Object object = l.get(i + offset);
			if (i > 0) {
				sb.append(", ");
			}
...
class RandomAccessSubList<E> extends SubList<E> implements RandomAccess {
	...
	RandomAccessSubList(final NSMutableArray<E> list, final int fromIndex, final int toIndex) {
