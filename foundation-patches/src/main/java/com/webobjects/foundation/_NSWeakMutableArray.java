...
public class _NSWeakMutableArray extends _NSWeakMutableCollection implements Serializable {
	...
	static {
		serialPersistentFields = new ObjectStreamField[] {
				new ObjectStreamField("objects", _NSUtilities._NoObjectArray.getClass()) };
	}
	public static final Class _CLASS = _NSUtilities
			._classWithFullySpecifiedName("com.webobjects.foundation._NSWeakMutableArray");
	...
	private final NSMutableArray array;

	public _NSWeakMutableArray() {
		this(16);
	}

	public _NSWeakMutableArray(final int capacity) {
	...
	public NSArray allObjects() {
		processQueue();
		final NSMutableArray list = new NSMutableArray(array.count());
		final Iterator iterator = array.iterator();
		do {
			if (!iterator.hasNext()) {
				break;
			}
			final Object object = ((WeakReference) iterator.next()).get();
			if (object != null) {
				list.add(object);
			}
		} while (true);
	...
	public int indexOfObject(final Object anObject) {
		processQueue();
		final int c = array.count();
		for (int i = 0; i < c; i++) {
	...
	@Override
	public void addReference(final WeakReference object) {
		processQueue();
	...
	@Override
	public void removeReference(final Object object) {
		processQueue();
		array.removeObject(object);
	}

	@Override
	protected void __removeReference(final Reference object) {
		array.removeObject(object);
	}
	...
	private void writeObject(final ObjectOutputStream s) throws IOException {
		...
		final Object values[] = allObjects().objects();
