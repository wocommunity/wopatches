...
public class EOAttribute extends EOProperty implements EOPropertyListEncoding, EOSQLExpression.SQLValue {
	...
	enum Characteristic {
		ExternalType("externalType"), ColumnName("columnName"), ReadOnly("readOnly"), ClassName("className"),
		ValueType("valueType"), Width("width"), Precision("precision"), Scale("scale"), WriteFormat("writeFormat"),
		ReadFormat("readFormat"), UserInfo("userInfo"), ServerTimeZone("serverTimeZone"),
		ValueFactoryClassName("valueFactoryClassName"), ValueFactoryMethodName("valueFactoryMethodName"),
		...
		ParameterDirection("parameterDirection"), InternalInfo("_internalInfo");

		private final String _externalName;

		Characteristic(final String externalName) {
		...
		public static Characteristic characteristicForName(final String value) {
	...
	public synchronized Map overwrittenCharacteristics() {
		if (_overwrittenCharacteristics == null) {
			final Map list = new EnumMap<>(Characteristic.class);
			for (final Characteristic c : Characteristic.values()) {
	...
	protected void _setOverrideForCharacteristic(final Characteristic key) {
		overwrittenCharacteristics().put(key, Boolean.TRUE);
	}

	public boolean overridesPrototypeDefinitionForCharacteristic(final Characteristic key) {
		final Boolean value = (Boolean) overwrittenCharacteristics().get(key);
	...
	protected boolean _hasAnyOverrides() {
		...
		for (final Object element : overwrittenCharacteristics().values()) {
			final Boolean anEnum = (Boolean) element;
	...
	public String _toString(final int indent) {
		String indentString = "";
		...
		String aKey = "";
		try {
			final NSMutableDictionary aDictionary = new NSMutableDictionary();
			encodeIntoPropertyList(aDictionary);
			for (final Enumeration enumerator = aDictionary.keyEnumerator(); enumerator.hasMoreElements(); aLog
					.append(new StringBuilder().append(indentString).append('\t').append(aKey).append(" : '")
							.append(aDictionary.objectForKey(aKey)).append("'\n").toString())) {
				aKey = (String) enumerator.nextElement();
			}

		} catch (final Exception exception) {
			aLog.append(new StringBuilder().append(indentString).append('\t').append(getClass().getName())
					.append(".toString: characteristics ").append(aKey).append(" exception ").append(exception)
					.append('\n').toString());
		}
	...
	private EOAttribute _parent_attributeNamed(final String name) {
		if (_parent != null) {
	...
	private void _parent_removeAttribute(final EOAttribute att) {
		if (_parent != null) {
	...
	public boolean isFlattened() {
		...
		final int count = _definitionArray.count();
		if (count < 2) {
			return false;
		}
		for (int index = 0; index < count - 1; index++) {
			final Object property = _definitionArray.objectAtIndex(index);
	...
	public boolean _isPrimaryKeyClassProperty() {
		final NSArray pkAtts = _parent_primaryKeyAttributes();
		if (pkAtts == null || !pkAtts.containsObject(this)) {
			return false;
		}
	...
	public EOAttribute(final NSDictionary plist, final Object owner) {
		_flags_isNonUpdateable = false;
		_flags_isNonUpdateableInitialized = false;
		...
		_valueTypeClassName = "";
		setParent(owner);
		String string = (String) plist.objectForKey("prototypeName");
		if (string != null) {
			setPrototype(_parentModel().prototypeAttributeNamed(string));
		}
		string = (String) plist.objectForKey("externalType");
		if (string != null) {
			setExternalType(string);
		}
		string = (String) plist.objectForKey("isReadOnly");
		if (string != null) {
			setReadOnly("Y".equals(string));
		}
		string = (String) plist.objectForKey("allowsNull");
		if (string != null || _prototypeName == null) {
			setAllowsNull("Y".equals(string));
		}
		string = (String) plist.objectForKey("valueType");
		if (string != null) {
			setValueType(string);
		}
		string = (String) plist.objectForKey("valueClassName");
		if (string != null) {
		...
		if (_width == 0 && _externalType != null) {
			final int leftParen = _externalType.indexOf("(");
			if (leftParen != -1) {
				final int rightParen = _externalType.indexOf(")");
				if (rightParen != -1) {
					final String sizeString = _externalType.substring(leftParen + 1, rightParen);
					int max;
					try {
		...
		final String tzName = (String) plist.objectForKey("serverTimeZone");
		if (tzName != null) {
	...
	private boolean shouldEncodeIvarWithPrototypeCharacteristic(final int ivar, final Characteristic key) {
		return ivar != 0 && (_prototype == null || overridesPrototypeDefinitionForCharacteristic(key));
	}

	private boolean shouldEncodeIvarWithPrototypeCharacteristic(final Object ivar, final Characteristic key) {
		return ivar != null && (_prototype == null || overridesPrototypeDefinitionForCharacteristic(key));
	}

	private boolean shouldEncodeScalarIvarWithPrototypeCharacteristic(final int scalarIvar, final Characteristic key) {
		return _prototype == null && scalarIvar != 0
				|| _prototype != null && overridesPrototypeDefinitionForCharacteristic(key);
	}

	@Override
	public void encodeIntoPropertyList(final NSMutableDictionary result) {
		if (_name != null) {
		...
		if (shouldEncodeIvarWithPrototypeCharacteristic(_valueFactoryMethodName,
				Characteristic.ValueFactoryMethodName)) {
			result.setObjectForKey(_valueFactoryMethodName, "valueFactoryMethodName");
		}
		if (shouldEncodeIvarWithPrototypeCharacteristic(_adaptorValueConversionClassName,
				Characteristic.AdaptorValueConversionClassName)) {
			result.setObjectForKey(_adaptorValueConversionClassName, "adaptorValueConversionClassName");
		}
		if (shouldEncodeIvarWithPrototypeCharacteristic(_adaptorValueConversionMethodName,
				Characteristic.AdaptorValueConversionMethodName)) {
	...
	@Override
	public void awakeWithPropertyList(final NSDictionary plist) {
		Object encoding;
		String string = (String) plist.objectForKey("definition");
		if (string != null) {
	...
	public boolean overridesPrototypeDefinitionForKey(final String key) {
		final Characteristic aCharacteristic = Characteristic.characteristicForName(key);
	...
	public void setPrototype(final EOAttribute prototype) {
	...
	public void setReadOnly(final boolean yn) {
	...
	private void _setReadOnly(final boolean yn) {
		if (_flags_isReadOnly != yn) {
			if (!yn && isDerived() && !isFlattened()) {
				throw new IllegalArgumentException("Unable to remove read only on a derived not flattened attribute");
			}
	...
	private void _setColumnName(final String columnName) {
		if (columnName == null && _columnName == null) {
			return;
		}
	...
	public void setExternalType(final String string) {
	...
	private void _setExternalType(final String string) {
		if (_externalType == null || !_externalType.equals(string)) {
	...
	public void setValueType(final String string) {
	...
	private void _setValueType(final String string) {
		if (_valueType == null || !_valueType.equals(string)) {
			_valueType = string == null || string.length() <= 0 ? null : string;
		}
	}

	private String _javaNameForObjcName(final String name) {
		if (name == null) {
			return null;
		}
		if (name.startsWith(CN_NSPrefix)) {
			if (CN_NSString.equals(name)) {
				return CN_JavaString;
			}
			if (CN_NSNumber.equals(name)) {
				return CN_JavaNumber;
			}
			if (CN_NSDecimalNumber.equals(name)) {
				return CN_JavaBigDecimal;
			}
			if (CN_NSCalendarDate.equals(name) || CN_NSGregorianDate.equals(name)) {
				return CN_JavaNSTimestamp;
			}
			if (CN_NSData.equals(name)) {
	...
	private String _objcNameForJavaName(final String name) {
		if (name == null) {
			return null;
		}
		if (CN_JavaString.equals(name)) {
			return CN_NSString;
		}
		if (CN_JavaNumber.equals(name)) {
			return CN_NSNumber;
		}
		if (CN_JavaBigDecimal.equals(name)) {
			return CN_NSDecimalNumber;
		}
		if (CN_JavaNSTimestamp.equals(name)) {
			return CN_NSCalendarDate;
		}
		if (CN_JavaNSData.equals(name)) {
			return CN_NSData;
		}
		if ("".equals(name)) {
			return null;
	...
	/**
	 * @param name valueClassName
	 * @deprecated Method setValueClassName is deprecated
	 */
	@Deprecated
	public void setValueClassName(final String name) {
	...
	private void _setClassName(final String name) {
	...
	public void setWidth(final int length) {
		_setWidth(length);
		_setOverrideForCharacteristic(Characteristic.Width);
	}

	private void _setWidth(final int length) {
		_width = length;
	}
	...
	private void _setPrecision(final int precision) {
	...
	private void _setScale(final int scale) {
		_scale = scale;
	}

	public void setAllowsNull(final boolean allowsNull) {
	...
	private void _setAllowsNull(final boolean allowsNull) {
		if (allowsNull == _flags_allowsNull) {
			return;
		}
	...
	public void setServerTimeZone(final TimeZone tz) {
		_setServerTimeZone(tz);
		_setOverrideForCharacteristic(Characteristic.ServerTimeZone);
	}

	private void _setServerTimeZone(final TimeZone tz) {
		_serverTimeZone = (TimeZone) tz.clone();
	}

	public Object newValueForBytes(final byte bytes[], final int length) {
		return newValueForImmutableBytes(bytes);
	}

	public Object newValueForImmutableBytes(final byte bytes[]) {
		final Class dataClass = NSData._CLASS;
		Object value = null;
		NSData data = null;
		if (_valueClass == null) {
		...
		case FactoryMethodArgumentIsData:
			try {
				value = _valueFactoryMethod.invoke(valueFactoryClass() == null ? _valueClass : valueFactoryClass(),
						data);
		...
		case FactoryMethodArgumentIsString:
			try {
				value = _valueFactoryMethod.invoke(valueFactoryClass() == null ? _valueClass : valueFactoryClass(),
						_NSStringUtilities.stringForBytes(bytes, StandardCharsets.UTF_8.name()));
				if (!NSLog.debugLoggingAllowedForLevelAndGroups(NSLog.DebugLevelInformational,
						NSLog.DebugGroupDatabaseAccess)) {
					break;
				}
				if (NSLog.debugLoggingAllowedForLevel(NSLog.DebugLevelDetailed)) {
					NSLog.debug.appendln(new RuntimeException(
							"Deprecated implicit bytes->String conversion.  Assuming UTF-8 encoding."));
				} else {
					NSLog.debug.appendln(
							"Deprecated implicit bytes->String conversion.  Assuming UTF-8 encoding.  Set debug level to NSLog.DebugLevelDetailed for more information.");
				}
			} catch (IllegalAccessException | IllegalArgumentException | InvocationTargetException
					| NoSuchMethodException e) {
				throw NSForwardException._runtimeExceptionForThrowable(e);
	 ...
	 */
	@Deprecated
	public Object newValueForBytesString(final byte bytes[], final int length) {
		...
		if (_valueClass == stringClass) {
			final String result = _NSStringUtilities.stringForBytes(bytes, 0, length, StandardCharsets.UTF_8.name());
			if (NSLog.debugLoggingAllowedForLevelAndGroups(NSLog.DebugLevelInformational,
					NSLog.DebugGroupDatabaseAccess)) {
				if (NSLog.debugLoggingAllowedForLevel(NSLog.DebugLevelDetailed)) {
					NSLog.debug.appendln(new RuntimeException(
		...
		if (_valueClass == stringClass || _argumentType == 1 || _valueFactoryMethod == null) {
			value = _NSStringUtilities.stringForBytes(bytes, 0, length, StandardCharsets.UTF_8.name());
			if (NSLog.debugLoggingAllowedForLevelAndGroups(NSLog.DebugLevelInformational,
					NSLog.DebugGroupDatabaseAccess)) {
				if (NSLog.debugLoggingAllowedForLevel(NSLog.DebugLevelDetailed)) {
					NSLog.debug.appendln(new RuntimeException(
		...
		case FactoryMethodArgumentIsString:
			try {
				value1 = _valueFactoryMethod.invoke(valueFactoryClass() == null ? _valueClass : valueFactoryClass(),
						value);
			} catch (IllegalAccessException | IllegalArgumentException | InvocationTargetException
					| NoSuchMethodException e) {
				throw NSForwardException._runtimeExceptionForThrowable(e);
		...
		case FactoryMethodArgumentIsBytes:
			try {
				value1 = _valueFactoryMethod.invoke(valueFactoryClass() == null ? _valueClass : valueFactoryClass(),
						new Object[] { bytes, _NSUtilities.IntegerForInt(length) });
			} catch (IllegalAccessException | IllegalArgumentException | InvocationTargetException
					| NoSuchMethodException e) {
				throw NSForwardException._runtimeExceptionForThrowable(e);
			}
	...
	public Object newValueForDate(final Object value) {
		if (valueFactoryMethod() == null) {
			if (value instanceof Timestamp) {
				return new NSTimestamp((Timestamp) value);
			}
			if (value instanceof Date) {
				final Date temp = (Date) value;
				return new NSTimestamp(temp.getTime());
			}
			throw new EOGeneralAdaptorException(new StringBuilder().append(value).append(" of type ")
					.append(value.getClass().getName())
					.append(" is not a valid Date type.  You must use java.sql.Timestamp, java.sql.Date, or java.sql.Time")
					.toString(), null);
		}
		if (!(value instanceof Date)) {
			throw new EOGeneralAdaptorException(new StringBuilder().append(value).append(" of type ")
		...
		try {
			if (valueFactoryClass() != null) {
				final Class<?> factoryClass = valueFactoryClass();
				return valueFactoryMethod().invoke(factoryClass, date);
			}
			final Class<?> c = _NSUtilities.classWithName(className());
	...
	public Object newValueForNumber(final Object value) {
		if (valueFactoryMethod() != null) {
			if (!(value instanceof Number)) {
				throw new EOGeneralAdaptorException(new StringBuilder().append(value).append(" of type ")
						.append(value.getClass().getName()).append(" is not a valid Number type.").toString(), null);
			}
			...
			try {
				...
				final NoSuchMethodException enhancedNsme = new NoSuchMethodException(
						e.getMessage() + " with parameters " + Arrays.toString(valueFactoryMethod().parameterTypes()));
	...
	public Object newValueForString(final String str) {
		final Class stringClass = String.class;
		Object value = null;
		if (_valueClass == null) {
			_valueClass = _NSUtilities.classWithName(className());// _valueClassName);
		...
		case FactoryMethodArgumentIsBytes:
			try {
				value = _valueFactoryMethod.invoke(valueFactoryClass() == null ? _valueClass : valueFactoryClass(),
						_NSStringUtilities.bytesForString(str, StandardCharsets.UTF_8.name()));
				if (!NSLog.debugLoggingAllowedForLevelAndGroups(NSLog.DebugLevelInformational,
						NSLog.DebugGroupDatabaseAccess)) {
					break;
				}
				if (NSLog.debugLoggingAllowedForLevel(NSLog.DebugLevelDetailed)) {
					NSLog.debug.appendln(new RuntimeException(
							"Deprecated implicit String->bytes conversion.  Assuming UTF-8 encoding."));
				} else {
					NSLog.debug.appendln(
							"Deprecated implicit String->bytes conversion.  Assuming UTF-8 encoding.  Set debug level to NSLog.DebugLevelDetailed for more information.");
				}
			} catch (IllegalAccessException | IllegalArgumentException | InvocationTargetException
					| NoSuchMethodException e) {
				throw NSForwardException._runtimeExceptionForThrowable(e);
		...
		case FactoryMethodArgumentIsData:
			try {
				value = _valueFactoryMethod.invoke(valueFactoryClass() == null ? _valueClass : valueFactoryClass(),
						new NSData(str, StandardCharsets.UTF_8.name()));
				if (!NSLog.debugLoggingAllowedForLevelAndGroups(NSLog.DebugLevelInformational,
						NSLog.DebugGroupDatabaseAccess)) {
					break;
				}
				if (NSLog.debugLoggingAllowedForLevel(NSLog.DebugLevelDetailed)) {
					NSLog.debug.appendln(new RuntimeException(
							"Deprecated implicit String->NSData conversion.  Assuming UTF-8 encoding."));
				} else {
					NSLog.debug.appendln(
							"Deprecated implicit String->NSData conversion.  Assuming UTF-8 encoding.  Set debug level to NSLog.DebugLevelDetailed for more information.");
				}
			} catch (IllegalAccessException | IllegalArgumentException | InvocationTargetException
					| NoSuchMethodException e) {
				throw NSForwardException._runtimeExceptionForThrowable(e);
	...
	public Object adaptorValueByConvertingAttributeValue(final Object value) {
		Object convertedValue = value;
		final NSSelector conversionMethod = adaptorValueConversionMethod();
		final Class conversionClass = adaptorValueConversionClass();
		...
		final int valueType = adaptorValueType();
		...
		if (FactoryMethodArgumentIsBytes == factoryMethodArgumentType() && valueFactoryMethod() != null) {
			final byte[] bytes = (byte[]) convertedValue;
			convertedValue = new NSData(bytes);
		}

		// Make an additional check here to support custom date types.
		if ((AdaptorDateType != valueType || conversionMethod == null || !(convertedValue instanceof Date))
				&& !valueClasses[valueType].isInstance(convertedValue)
				&& (valueClasses[valueType] != Number.class || !(convertedValue instanceof Boolean))) {
			final EOEntity parentEntity = (EOEntity) parent();
			final String entityName = parentEntity == null ? "<unspecified>" : parentEntity.name();
			throw new IllegalArgumentException(new StringBuilder().append(
					"EOAttribute adaptorValueByConvertingAttributeValue(Object): Unable to convert value of class ")
					.append(convertedValue.getClass().getName()).append(" for attribute '").append(name())
					.append("' in entity '").append(entityName).append("' to adaptor type EOAttribute.Adaptor")
					.append(valueTypeNames[valueType]).append("Type.  Check the signature of the conversion method ")
					.append(className()).append('.')
					.append(conversionMethod == null ? "NotFound" : conversionMethod.name()).append("().").toString());
		}
	...
	public int adaptorValueType() {
		...
		final Class valueClass = _NSUtilities.classWithName(className);
		if (valueClass == null) {
			throw new IllegalStateException(new StringBuilder().append("adaptorValueType: unable to load class named '")
					.append(className).append("' for attribute ").append(name()).append(" on entity ")
					.append(entity().name()).toString());
		}
	...
	public Class adaptorValueClass() {
		...
		case AdaptorNumberType:
			...
			if (NSLog.debugLoggingAllowedForLevel(NSLog.DebugLevelInformational)) {
				NSLog.debug.appendln(new StringBuilder().append("value type '").append(_valueTypeChar())
						.append("' on attribute \"").append(name()).append("\" of entity \"").append(entity().name())
		...
		case AdaptorBytesType:
			if (adaptorValueConversionMethod() != null
					&& factoryMethodArgumentType() == FactoryMethodArgumentIsString) {
				return String.class;
			}
						...
						if (_NSUtilities._isClassANumberOrABoolean(valueClass)) {
							...
							default:
								if (NSLog.debugLoggingAllowedForLevel(2)) {
									NSLog.debug.appendln(new StringBuilder().append("value type '")
											.append(_valueTypeChar()).append("' on attribute \"").append(name())
											.append("\" of entity \"").append(entity().name())
											.append("\" is invalid.\nYou should fix your EOModel.").toString());
								}
								valueClass = Integer.class;
								break;
	...
	public void setFactoryMethodArgumentType(final int argumentType) {
		_setFactoryMethodArgumentType(argumentType);
		_setOverrideForCharacteristic(Characteristic.FactoryMethodArgumentType);
	}

	private void _setFactoryMethodArgumentType(final int argumentType) {
	...
	public void setValueFactoryClassName(final String factoryClassName) {
		_setValueFactoryClassName(factoryClassName);
		_setOverrideForCharacteristic(Characteristic.ValueFactoryClassName);
	}

	private void _setValueFactoryClassName(final String factoryClassName) {
	...
	public void setValueFactoryMethodName(final String factoryMethodName) {
		_setValueFactoryMethodName(factoryMethodName);
		_setOverrideForCharacteristic(Characteristic.ValueFactoryMethodName);
	}

	private void _setValueFactoryMethodName(final String factoryMethodName) {
	...
	public void setAdaptorValueConversionClassName(final String conversionClassName) {
	...
	private void _setAdaptorValueConversionClassName(final String conversionClassName) {
	...
	public void setAdaptorValueConversionMethodName(final String conversionMethodName) {
	...
	private void _setAdaptorValueConversionMethodName(final String conversionMethodName) {
		if (conversionMethodName != null && conversionMethodName.length() != 0) {
			...
			if (adaptorValueConversionClass() != null) {
				final Class valueClass = _NSUtilities.classWithName(className());
				_adaptorValueConversionMethod = new NSSelector(_adaptorValueConversionMethodName,
	...
	public Object validateValue(final Object valueP) throws com.webobjects.foundation.NSValidation.ValidationException {
		Object value = valueP;
		if (value == null || value == NSKeyValueCoding.NullValue) {
			...
			throw new com.webobjects.foundation.NSValidation.ValidationException(
					new StringBuilder().append("The ").append(name()).append(" property of ").append(entity().name())
							.append(" is not allowed to be null.").toString(),
					this, name());
		}
		...
		final Class aClass = _NSUtilities.classWithName(className);
		if (aClass == null) {
			if (NSLog.debugLoggingAllowedForLevelAndGroups(2, 128L)) {
				NSLog.debug.appendln(new StringBuilder().append("Unable to find the class ").append(className).append(
						" in the Java runtime. Unable to validate. Please check your CLASSPATH, or the type for attribute ")
		...
		if (!aClass.isInstance(value)) {
			...
			if (!aClass.isInstance(value)) {
				...
				try {
					...
					throw new com.webobjects.foundation.NSValidation.ValidationException(new StringBuilder()
							.append("Error encountered converting value of class ").append(value.getClass().getName())
							.append(" to type specified in attribute '").append(name()).append("' of entity '")
							.append(entity().name()).append('\'').toString(), this, name());
				}
			}
		}
		if (value == null) {
			throw new com.webobjects.foundation.NSValidation.ValidationException(
					new StringBuilder()
							.append("Error encountered converting null value to type specified in attribute '")
							.append(name()).append("' of entity '").append(entity().name()).append('\'').toString(),
					this, name());
		}
		final int maxLen = width();
		if (maxLen != 0) {
			int currLength = 0;
			...
			try {
				...
				throw new com.webobjects.foundation.NSValidation.ValidationException(
						new StringBuilder().append("Error encountered converting value of class ")
								.append(value.getClass().getName()).append(" to type specified in attribute '")
								.append(name()).append("' of entity '").append(entity().name())
								.append("'. Conversion exception is : ").append(e.getMessage()).toString(),
						this, name());
			}
			...
			if (currLength > maxLen) {
				throw new com.webobjects.foundation.NSValidation.ValidationException(new StringBuilder().append("The ")
						.append(name()).append(" property of ").append(entity().name())
						.append(" exceeds maximum length of ").append(maxLen).append(" characters").toString(), this,
						name());
			}
		}
		return value;
	...
	boolean referencesProperty(final Object property) {
		if (_definitionArray == null) {
			return false;
		}
	...
	void setParent(final Object parent) {
		if (parent == null) {
			...
			throw new IllegalArgumentException(
					new StringBuilder().append("Invalid object of type ").append(parent.getClass().getName())
							.append(" passed to setParent (expected EOEntity or EOStoredProcedure").toString());
		}
	}

	void setEntity(final EOEntity entity) {
		if (_parent == entity) {
			return;
		}
	...
	public void _setSourceToDestinationKeyMap(final NSDictionary mapping) {
	...
	public String relationshipPath() {
		...
		final int iCount = _definitionArray.count() - 1;
	...
	public String _setNewFormatStringOld(final String newString, final String old) {
	...
	public int _factoryMethodArgumentTypeFromString(final String string) {
		final String aString = (string == null ? "" : string).toLowerCase();
		...
		return !aString.equals("EOFactoryMethodArgumentIsNSString".toLowerCase()) ? FactoryMethodArgumentIsData
				: FactoryMethodArgumentIsString;
	}

	public String _stringForFactoryMethodArgumentType(final int type) {
		switch (type) {
		case FactoryMethodArgumentIsBytes:
	...
	protected void _setDefinitionArray(final _EOExpressionArray definitionArray) {
	...
	protected void _setDefinitionWithoutFlushingCaches(final String definition) {
		if (_parent != null) {
			_columnName = null;
			if (definition == null) {
				_definitionArray = null;
	...
	protected void _setValuesFromTargetAttribute() {
		if (isFlattened()) {
			final EOAttribute property = (EOAttribute) _definitionArray.lastObject();
			...
			final int adaptorDataType = property.adaptorValueType();
	...
	protected Object _objectForPlist(final Object plist) {
		...
		final NSDictionary pl = (NSDictionary) plist;
		String string = (String) pl.objectForKey("name");
		if (string != null || (string = (String) pl.objectForKey("path")) != null) {
			return entity()._parsePropertyName(string);
		}
		final NSArray array = (NSArray) pl.objectForKey("array");
		if (array == null) {
			return null;
		}
		final _EOExpressionArray result = new _EOExpressionArray();
		string = (String) pl.objectForKey("prefix");
		if (string != null) {
			result.setPrefix(string);
		}
		string = (String) pl.objectForKey("infix");
		if (string != null) {
			result.setInfix(string);
		}
		string = (String) pl.objectForKey("suffix");
		if (string != null) {
	...
	protected void _removeFromEntityArraySelector(final NSArray oldArray, final NSSelector sel) {
		if (oldArray.indexOfObject(this) != -1) {
			final NSMutableArray newArray = new NSMutableArray(oldArray);
	...
	protected Object _normalizeDefinitionPath(final Object definition, final NSArray attributePath) {
		NSArray path = null;
		...
		if (definition instanceof EOAttribute) {
			final EOAttribute attribute = (EOAttribute) definition;
			if (attribute == this) {
				return null;
			}
			if (attribute.isDerived()) {
				final _EOExpressionArray attrDef = attribute._definitionArray();
				...
				final _EOExpressionArray newPath = new _EOExpressionArray();
				newPath.setInfix(".");
		...
		if (((_EOExpressionArray) definition)._isPropertyPath()) {
			...
			final EOAttribute attribute = (EOAttribute) ((_EOExpressionArray) definition).lastObject();
			if (attribute.isDerived()) {
				final _EOExpressionArray attrDef = attribute._definitionArray();
		...
		for (int i = 0; i < cnt; i++) {
			final Object elem = ((_EOExpressionArray) definition).objectAtIndex(i);
			final Object nElem = _normalizeDefinitionPath(elem, path);
			if (nElem == null) {
				return null;
			}
			if (nElem instanceof _EOExpressionArray && !((_EOExpressionArray) nElem)._isPropertyPath()) {
				final int jcnt = ((_EOExpressionArray) nElem).count();
	...
	protected EOAttribute(final EOEntity entity, final String definition) {
		this();
		_name = definition;
		setParent(entity);
