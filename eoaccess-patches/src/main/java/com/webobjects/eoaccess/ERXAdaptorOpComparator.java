...
public class ERXAdaptorOpComparator extends EOAdaptorOpComparator {
	 ...
	 */
	public ERXAdaptorOpComparator(final NSArray entityNameOrdering) {
	...
	public int compare(final Object object1, final Object object2)
		...
		if (object1 == null ||
			...
			throw new com.webobjects.foundation.NSComparator.ComparisonException(
					"Unable to compare objects. Objects should be instance of class EOAdaptorOperation. Comparison was made with "
							+ object1 + " and " + object2 + ".");
		}

		final EOAdaptorOperation a = (EOAdaptorOperation) object1;
		final EOAdaptorOperation b = (EOAdaptorOperation) object2;
		...
		final NSArray entityNames = _context;
		...
		final int aPriority = entityNames == null ? 0 : entityNames.indexOfObject(entityNameA);
		final int bPriority = entityNames == null ? 0 : entityNames.indexOfObject(entityNameB);
		int order = 1;
	...
	/**
	 * Returns the name of EOEntity updated by adaptorOp. If this is updating
	 * through a flattened attribute, the name returned is the Entity of the target
	 * attribute.
	 *
	 * @param adaptorOp EOAdaptorOperation to return Entity name from
	 * @return name of EOEntity updated by adaptorOp
	 */
	protected String entityNameFromAdaptorOperation(final EOAdaptorOperation adaptorOp) {
		final EOEntity entity = adaptorOp.entity();

		// If there are no changed valued (e.g. EOAdaptorDeleteOperator), use the
		// standard entity
		if (adaptorOp.changedValues() == null) {
			return entity.name();
		}

		// Flattened attributes are grouped together by entity so any attribute in
		// changedValues should work
		final NSArray<String> attrNames = adaptorOp.changedValues().allKeys();
		for (int i = 0; i < attrNames.count(); i++) {
			final EOAttribute attribute = entity.attributeNamed(attrNames.objectAtIndex(i));
			/*
			 * Sometimes attributes are named like "NeededByEOF0" for vertical inheritance.
			 * With single table inheritance, attributes names from other entities may
			 * appear. Better safe than sorry :) Keep checking until we find an attribute on
			 * the entity for the given attribute name.
			 *
	...
	private int decodeAdaptorTypeForEntityAndStoredProcedure(final EOEntity entity, final EOStoredProcedure sp) {
