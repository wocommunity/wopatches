...
/**
 * General purpose constant class, useful when you want reference object that
 * are not bytes or strings in the DB like what you get with the factory
 * classes. If you use objects of this class, you might be able to completely
 * remove the EOSharedEditingContext (the google search term for "why does my
 * app lock up").
 * <p>
 * To use the Number constants, you need to add an entry
 * <code>ERXConstantClassName=Test.Status</code> to the attribute's userInfo in
 * question and your EO's class description needs to be a
 * {@link ERXEntityClassDescription}, also you must enable the
 * {@link er.extensions.jdbc.ERXJDBCAdaptor}.
 * <p>
 ...
 * </pre>
 *
 * Note that upon class initialization 2500 Integers will be created and cached,
 * from 0 - 2499.
 */
public abstract class ERXConstant {
	...
	public static NSArray constantsForClassName(String clazzName) {
		synchronized (_store) {
			...
			NSArray<EOSortOrdering> order = new NSArray<>(new EOSortOrdering[] {
					EOSortOrdering.sortOrderingWithKey("sortOrder", EOSortOrdering.CompareCaseInsensitiveAscending) });
	...
	public static Constant constantForClassNamed(Object value, String clazzName) {
		synchronized (_store) {
			...
			if (NSLog.debugLoggingAllowedForLevel(NSLog.DebugLevelDetailed)) {
				NSLog.out.appendln("Getting " + result + " for " + clazzName + " and " + value);
			}
			return result;
		}
	}
	...
	private static void registerConstant(Object key, Constant value, Class clazz) {
		synchronized (_store) {
			...
			if (NSLog.debugLoggingAllowedForLevel(NSLog.DebugLevelDetailed)) {
				NSLog.out.appendln("Putting " + key + " for " + className);
			}
	...
	public static class NumberConstant extends Number implements Constant {
		...
		/**
		 * Sets the value and puts the object in the store keyed by class name and
		 * value.
		 *
		 * @param value
		 */
		...
		public final boolean equals(Object otherObject) {
			/*
			 * AK: we would violate the equals contract here, but we may need this with D2W
			 * later? if((otherObject instanceof Number) && !(otherObject instanceof
			 * ERXConstant)) { return ((Number)otherObject).intValue() == intValue(); }
			 */
	...
	public static abstract class ByteConstant implements Constant {
		...
		public ByteConstant(String value, String name) {
			// AK: making a lot of assumptions here... the value is a <hex data>, the result
			// is a valid NSData
	...
	public static final Integer MinusOneInteger = -1;
	...
	public static final Class[] StringClassArray = { String.class };
	public static final Object[] EmptyObjectArray = {};
	/** an empty gif image */
	public static final NSData EmptyImage = (NSData) NSPropertyListSerialization.propertyListFromString(
			"<47494638396101000100800000ffffff00000021f90401000000002c00000000010001000002024401003b00>");
	...
	public static Integer integerForInt(int i) {
		return i >= 0 && i < MAX_INT ? INTEGERS[i] : i;
	}
